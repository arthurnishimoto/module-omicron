...
public class ScreenConfigCalc : MonoBehaviour {
	...
	void Update () {
        ...
        if (screenMask)
        {
            switch (renderMode)
            {
                case (RenderMode.Background): screenMask.GetComponent<Renderer>().sharedMaterial.SetFloat("_ZTest", 2); break;
                case (RenderMode.None): screenMask.GetComponent<Renderer>().sharedMaterial.SetFloat("_ZTest", 1); break;
                case (RenderMode.Overlay): screenMask.GetComponent<Renderer>().sharedMaterial.SetFloat("_ZTest", 0); break;
            }
        }
    }
}
